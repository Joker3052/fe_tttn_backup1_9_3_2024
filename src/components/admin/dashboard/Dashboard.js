...
// Hàm dùng để tạo dữ liệu giả
const generateFakeData = () => {
  return Array.from({ length: 12 }, (_, index) => ({
    label: `Label ${index + 1}`,
    men_price: Math.floor(Math.random() * 1000),
    women_price: Math.floor(Math.random() * 1000),
    men_product: Math.floor(Math.random() * 50),
    women_product: Math.floor(Math.random() * 50),
  }));
};
...
function Dashboard() {
  ...
  const getdashboard = () => {
    ...
    setListdashboard(fakeData);
    setOriginalListdashboard(fakeData);
    setData(fakeData.map(item => ({
      label: item.label,
